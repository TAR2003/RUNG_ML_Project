...
parser.add_argument('--attack_steps', type=int, default=100,
                    help='Number of PGD attack iterations (default: 100).')
parser.add_argument('--attack_epochs', type=int, default=None,
                    help='Alias for --attack_steps (if set, overrides --attack_steps).')
parser.add_argument('--beta', type=float, default=1.0,
                    help='Stealth weight for cosine loss term (default: 1.0).')
...
def global_evasion_pgd_cosine(
    ...
    def loss_fn(flip):
        ...
        # Minimize margin (same direction as base PGD implementation).
    ...
    for model_name, custom_model_params, custom_fit_params in attack_configs:
        ...
        cleans, accs,  edge_flips, models = rep_global_evasion(
            ...
            *get_model(
            ...
            attack_method=global_evasion_pgd_cosine,
...
if __name__ == '__main__':
    os.makedirs(path+f'log/{args.data}/cosine_attack', exist_ok=True)
    args.attack_steps = args.attack_epochs if args.attack_epochs is not None else args.attack_steps
    ...
    sys.stdout = open(path+f'log/{args.data}/cosine_attack/{log_identifier}.log', 'w', buffering=1)

    get_model = get_model_default
    attack_name = "global_evasion_PGD_cosine"
